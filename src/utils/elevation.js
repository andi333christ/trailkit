...
export function computePathElevationProfile(segments) {
  ...
  for (const seg of segments) {
    ...
    for (let i = 0; i < geom.length; i++) {
      if (i > 0) cumDistM += haversineM([geom[i-1][0], geom[i-1][1]], [geom[i][0], geom[i][1]])
      const t = segTotal > 0 ? segDists[i] / segTotal : i / Math.max(geom.length - 1, 1)
      let eleM
      if (t <= upFrac) {
      ...
      result.push({ distKm: cumDistM / 1000, eleM: Math.round(eleM) })
    }
